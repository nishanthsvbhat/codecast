...
import type { CodeAlert, CodeInfo, CodeType } from '@/lib/types';
...
interface CodeAlertCardProps {
  code: CodeAlert;
  onAcknowledge: (id: string) => void;
}

const codeInfoMap: Record<CodeType, CodeInfo> = {
...
export default function CodeAlertCard({ code, onAcknowledge }: CodeAlertCardProps) {
  ...
  const cardClasses = cn(
    ...
    code.acknowledged ? 'bg-card/60 opacity-70' : 'bg-card'
  );
      ...
      <Card className={cardClasses}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            {!code.acknowledged && (
              <Button onClick={handleAcknowledgeClick} disabled={isAcknowledging} size="sm">
                {isAcknowledging ? (
      ...
      <Dialog open={isSummaryOpen} onOpenChange={setIsSummaryOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          {isLoadingSummary ? (
            ...
            <div className="py-4 text-sm whitespace-pre-wrap">{summary}</div>
          )}
        </DialogContent>
      </Dialog>
    </>
