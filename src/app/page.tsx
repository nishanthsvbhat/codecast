...
import type { CodeAlert } from '@/lib/types';
import CodeAlertCard from '@/components/code-alert-card';
import { Separator } from '@/components/ui/separator';

export default function Home() {
  const [codes, setCodes] = useState<CodeAlert[]>(initialCodes);

  const handleAcknowledge = useCallback((id: string) => {
    setCodes((prevCodes) =>
      prevCodes.map((code) =>
        code.id === id ? { ...code, acknowledged: true, acknowledgedAt: new Date() } : code
      )
    );
  }, []);
            ...
            {activeCodes.length > 0 ? (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {activeCodes.map((code) => (
                  <CodeAlertCard key={code.id} code={code} onAcknowledge={handleAcknowledge} />
                ))}
              </div>
            ) : (
